...
export function getBaseWebpackPartial(options: IBuildNodeBuilderOptions, context?: BuilderContext): Configuration {
  const { options: compilerOptions } = readTsConfig(options.tsConfig);
  const supportsEs2015 =
    compilerOptions.target !== ts.ScriptTarget.ES3 && compilerOptions.target !== ts.ScriptTarget.ES5;
...
function getAliases(options: IBuildNodeBuilderOptions): { [key: string]: string } {
...
function getStatsConfig(options: IBuildNodeBuilderOptions): Stats.ToStringOptions {
  return {
    hash: true,
    timings: false,
    cached: false,
